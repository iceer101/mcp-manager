// Adapters for the MCP config formats.
//
...
// Cursor         : ~/.cursor/mcp.json -> .mcpServers.{name} (global scope)
//
// Claude Desktop / Claude Code / Cursor have no native "disabled" flag in the file, so a disabled
...
// optional: a missing file reads as an empty config and is created on the first write.
function readJson(file, { optional } = {}) {
  if (optional && !fs.existsSync(file)) return { data: {}, indent: '  ', trailingNl: true };
...
// ---------- JSON adapter (Claude Desktop, Claude Code user + project scopes, Cursor) ----------

function jsonAdapter({ id, label, client, file, scope, containerPath, optional }) {
  const readJson_ = () => readJson(file, { optional });
...
function codexAdapter() {
  ...
  // A missing config.toml reads as empty and is created on the first write.
...
// A client is shown only when it is installed (its config directory exists);
// its config file itself may not exist yet and is created on the first write.
const installed = (...paths) => paths.some(p => fs.existsSync(p));

function getSources() {
  const sources = [];
  if (installed(path.dirname(FILES.desktop))) {
    sources.push(jsonAdapter({ id: 'desktop', label: 'Claude Desktop', client: 'desktop', file: FILES.desktop, scope: 'user', containerPath: ['mcpServers'], optional: true }));
  }
  if (installed(FILES.code, path.join(HOME, '.claude'))) {
    sources.push(jsonAdapter({ id: 'code', label: 'Claude Code', client: 'code', file: FILES.code, scope: 'user', containerPath: ['mcpServers'], optional: true }));
  }
  ...
  if (installed(path.dirname(FILES.codex))) sources.push(codexAdapter());
  if (installed(path.dirname(FILES.cursor))) {
    sources.push(jsonAdapter({ id: 'cursor', label: 'Cursor', client: 'cursor', file: FILES.cursor, scope: 'user', containerPath: ['mcpServers'], optional: true }));
  }
  return sources;
}
...
function fromNeutral(target, n) {
  ...
  } else if (target.client === 'cursor') {
    // Cursor picks Streamable HTTP or SSE by itself from the URL.
    out.url = n.url;
    if (hasHeaders) out.headers = n.headers;
  } else {
...
function saveUnified({ name, oldName, neutral, targets }) {
  ...
  for (const src of getSources()) {
    const isTarget = targets.includes(src.id);
    let existing;
    try { existing = src.list().find(s => s.name === from); }
    catch (e) {
      // An unreadable client must not block saving to the others; it is already flagged in the list.
      if (isTarget) throw new Error(`${src.label}: ${e.message}`);
      continue;
    }
    if (isTarget) {
...
module.exports = {
  getSources, getSource, snapshot, toNeutral, fromNeutral, comparable, saveUnified, getRaw, saveRaw,
};
